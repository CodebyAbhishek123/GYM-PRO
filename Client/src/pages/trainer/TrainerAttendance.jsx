...
import { getAllAttendance } from "../../services/admin.service";
...
const TrainerAttendance = () => {
  ...
  return (
    <div className="space-y-6">
      <div>
        <h1 className="text-2xl font-bold text-slate-950 dark:text-white">Trainee Attendance</h1>
        <p className="text-sm text-slate-500 dark:text-slate-400">View check-in histories and training schedules of members.</p>
      </div>
            ...
            <table className="w-full text-left border-collapse">
              <thead>
                <tr className="border-b border-slate-200 dark:border-slate-800 text-slate-400 text-sm font-semibold bg-slate-50/70 dark:bg-slate-850">
                  <th className="py-4 px-6">Trainee</th>
              ...
              <tbody>
                {filteredLogs.map(log => (
                  <tr key={log._id} className="border-b border-slate-100 dark:border-slate-800 last:border-0 hover:bg-slate-50 dark:hover:bg-slate-800/40 text-sm transition-colors duration-150">
                    ...
                    <td className="py-4 px-6 flex items-center gap-1.5 text-slate-600 dark:text-slate-400 text-xs font-semibold">
                      <FiClock />
